<template>
    <section class="space-y-4" aria-labelledby="upload-heading">
        ...
        >
            ...
            <span class="mt-3 text-sm font-medium text-slate-700">Select files</span>
            <span class="mt-1 text-xs text-slate-500">Drop one or more CSV files, or browse from your computer.</span>
        </label>
        <div v-if="datasetStore.uploadFiles.length" class="space-y-1 text-sm text-slate-600">
            ...
            <ul class="space-y-1">
                <li
                    ...
                    class="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs shadow-sm"
                >
